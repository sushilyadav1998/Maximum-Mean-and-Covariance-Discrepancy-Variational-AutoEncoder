# -*- coding: utf-8 -*-
"""
Created on Mon Nov  9 19:20:50 2020
...
train = IMAGES[:-50] 
test = IMAGES[-50:]
x_i = np.array([i[0] for i in train]).reshape(-1, Img_size, Img_size, 1)
y_i = np.array([i[1] for i in train])
x_train=x_i
y_train=y_i
...
def compute_kernel(x, y):
    ...
    #return tf.exp(-tf.reduce_mean(tf.square(tiled_x - tiled_y), axis=2) / tf.cast(dim, tf.float32))   #gaussian Kernel
    return tf.exp(-tf.reduce_mean(tiled_x - tiled_y))      #Exponential Kernel
    #return (tf.square(tf.tensordot(tf.transpose(tiled_y), tiled_x) + 1))   #Polynomial Kernel of Degree 2
...
checkpoint_vae = ModelCheckpoint(os.path.join('weightskl200epoch.h5'), save_weights_only = True, verbose=1)
...
#%%
d2=r'C:\Users\sushilkumar.yadav\Desktop\vmware\Personal\Research\Image_recognition_in_wild_using_Deep_Learning\Database_FR\self_images\Varun_Dhawan'
data_test = ImageDataGenerator(rescale=1./255).flow_from_directory(d2, 
                                                                   target_size = INPUT_DIM[:2],
                                                                   shuffle = True,
                                                                   class_mode = 'input',
                                                                   subset = 'training'
                                                                   )
example_batch = next(data_test)
example_batch = example_batch[0]
example_images = example_batch[:3]
#example_batch = data_test
#example_batch = example_batch[0]
...
def plot_compare_vae(images=None, add_noise=False):
  ...
  for i in range(n_to_show):
      ...
      sub.axis('off')        
      sub.imshow(img)
      #cv2.imwrite(r"C:\Users\sushilkumar.yadav\Desktop\vmware\Personal\Research\Image_recognition_in_wild_using_Deep_Learning\Database_FR\mmcd_vae_cnn\original_testing_dataset\original_testing_combined_dataset\Varun_Dhawan."+ str(i)+".jpeg", 255*img)
  ...
  for i in range(n_to_show):
      ...
      sub.axis('off')
      sub.imshow(img)
      cv2.imwrite(r"C:\Users\sushilkumar.yadav\Desktop\vmware\Personal\Research\Image_recognition_in_wild_using_Deep_Learning\Database_FR\self_images\Varun_Dhawan\Varun_Dhawan_klvae."+ str(i)+".jpeg", 255*img)
...
def vae_generate_images(n_to_show=10):
  reconst_images = vae_decoder.predict(np.random.normal(1,1,size=(n_to_show,Z_DIM)))
...
mscatter(z_mean[:, 0], z_mean[:, 1], s=100, edgecolors='black')
#plt.colorbar()
plt.xlabel("z[0]")
plt.ylabel("z[1]")
plt.show()
